...
> Site HTML statique (+ Vercel serverless) de Jérémy Sagnier. Vitrine perso, deux veilles IA/Business auto-générées, bibliothèque de tutos Codex, téléchargements gratuits.
>
...
- ❌ **"GMF", "chez GMF", "produit GMF"** → toujours remplacer par "en assurance" / "outil de vente" / formulation neutre. Article renommé `outil-vente-Codex.html` avec redirect 301 depuis l'ancien slug. AUCUN texte visible ne doit contenir "GMF". Raison : anonymisation du nom de l'employeur de Shirley (décidée 2026-04-23).
...
4. **Étape 02 · Passer à Codex** (fuchsia) — 2 cards : Codex.html + loops-Codex
...
Entre étapes : mini-marquees narratifs (Bases posées → Codex → Agents → Plus loin).
...
|---|---|---|---|---|
| 01 | `loops-Codex.html` | Les loops Codex, expliqués | 7 min | teal |
...
3. **Dropdown « Autres IA »** : Codex, Gemini, Le Chat (Mistral), Copilot, ou copy seul
...
**Pourquoi pas de boutons directs pour Codex/Gemini/Mistral/Copilot ?**
- Codex web a perdu son `?q=` en octobre 2025 (régression CVE prompt injection)
...
4. **Vérifier** :
   ...
   - Vérifier que la barre `.ai-summarize` apparaît juste après le `.tldr`. Cliquer « Perplexity » doit ouvrir un onglet avec un résumé en cours. Cliquer « Autres IA → Codex » doit copier le prompt et ouvrir Codex.ai dans un nouvel onglet.
...
### Downloads
- `downloads/AGENTS.md` — version **anonymisée** du global de Jérémy
- `downloads/skills/` — 26 skills custom (officiels Anthropic exclus)
- `downloads/README.md` — guide install
- `downloads/jeremy-Codex-pack.zip` — 690 Ko, pack complet
...
### Plugins Codex installés (scope user)
`superpowers` · `Codex-md-management` · `frontend-design` · `context7` · `code-review` · `code-simplifier` · `telegram`. Gestion via `Codex plugin <install|list|disable>`. Commandes slash dispo après restart.
...
**Dans le frontmatter du draft, utilise un `slug` court et propre** (ex: `skills-Codex-non-dev`). Le fichier draft porte le nom du `id` backlog pour matcher l'idée ; le slug publié est celui du frontmatter.

### Avant toute modif
- Lire ce AGENTS.md en entier
...
**cleanUrls: true** · `/Codex` résout `/Codex.html` automatiquement. Tous les liens internes peuvent omettre l'extension.
