...
function publicUrl(term) {
  if (ESSENTIALS_ANCHORS.has(term.slug)) return `${SITE_URL}/lexique-essentiels#${term.slug}`;
...
function renderExpressCards(terms) {
  const cards = terms
    ...
    .map((term) => `      <article class="lex-card" id="${escapeAttr(term.slug)}">
        ...
        <p class="lex-card-where"><strong>Où tu le croises</strong> : ${escapeHtml(term.where)}</p>
      </article>`)
    .join("\n\n");
...
function renderTermPage(term, allTerms) {
  ...
  // Ajouter FAQPage si des Q/A existent
  if (faq.length > 0) {
    graphItems.push({
      "@type": "FAQPage",
      "@id": `${canonical}#faq`,
...
async function updateHub(data) {
  ...
  if (html.includes(EXPRESS_START) && html.includes(EXPRESS_END)) {
    ...
    const nextHtml = html.replace(
      /(<div class="lex-express-grid">\n)[\s\S]*?(\n\s*<\/div>\n\s*<\/div>\n<\/section>\n\n<!-- Mini-marquee avant le CTA final -->)/,
      `$1${expressCards}$2`,
    );
    if (nextHtml === html) {
      throw new Error("Bloc .lex-express-grid introuvable dans lexique.html");
    }
    html = nextHtml;
  }
  html = html.replace(
